...
namespace ShiftPlan.Blazor.WebAssembly.Identity;
...
public class CookieAuthenticationStateProvider(IHttpClientFactory httpClientFactory) : AuthenticationStateProvider, IAccountManagement
{
	...
	public bool Auth => this.authenticated;
	...
	public override async Task<AuthenticationState> GetAuthenticationStateAsync()
	{
		...
		try
		{
			var msg = new HttpRequestMessage(HttpMethod.Get, "api/identity/manage/info");
			// the user info endpoint is secured, so if the user isn't logged in this will fail
			var userResponse = await this.httpClient.SendAsync(msg);
			...
			if (userInfo != null)
			{
				...
				var rolesResponse = await this.httpClient.GetAsync("/api/identity/roles");
				...
				var roles = JsonSerializer.Deserialize<RoleClaim[]>(rolesJson, this.jsonSerializerOptions);
	...
	public async Task LogoutAsync()
	{
		...
		await this.httpClient.PostAsync("/api/identity/logout", emptyContent);
		NotifyAuthenticationStateChanged(GetAuthenticationStateAsync());
	}
